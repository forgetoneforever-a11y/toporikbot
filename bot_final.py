...
class Database:
    ...
    async def get_videos(self):
        def op(db):
            return db.execute(
                "SELECT id, file_id, caption, category FROM videos ORDER BY id"
            ).fetchall()
    ...
    async def add_video(self, file_id: str, caption: Optional[str], category: str):
        def op(db):
            cur = db.execute(
                """
                INSERT INTO videos (file_id, caption, category)
                VALUES (?, ?, ?)
                """,
...
async def refresh_video_cache():
    global VIDEO_CACHE, VIDEO_CACHE_BY_CATEGORY
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🎬 Еще видео",
                    callback_data=f"next_v_{category}",
                )
            ],
            [
                InlineKeyboardButton(
